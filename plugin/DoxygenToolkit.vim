...
let s:licenseBoost = "          Copyright (C) \<enter>\<enter>"
let s:licenseBoost = s:licenseBoost . "Distributed under the Boost Software License, Version 1.0.\<enter>"
let s:licenseBoost = s:licenseBoost . "(See accompanying file LICENSE_1_0.txt or copy at\<enter>"
let s:licenseBoost = s:licenseBoost . " http://www.boost.org/LICENSE_1_0.txt)\<enter>\<enter>"
...
if !exists("g:DoxygenToolkit_licenseBoost")                        " Boost licence
	let g:DoxygenToolkit_licenseBoost = s:licenseBoost
endif
...
"""""""""""""""""""""""""""""""
" Doxygen Boost license comment
"""""""""""""""""""""""""""""""
function! <SID>DoxygenLicenseBooFunc()
  call s:InitializeParameters()
  ...
  let l:date = strftime("%Y")
  exec "normal O".strpart( s:startCommentBlock, 0, 1 )
  exec "normal A".strpart( s:startCommentBlock, 1 ).substitute( g:DoxygenToolkit_licenseBoost, "\<enter>", "\<enter>".s:interCommentBlock, "g" )
  if( s:endCommentBlock != "" )
    exec "normal o".s:endCommentBlock
  endif
  if( g:DoxygenToolkit_licenseBoost == s:licenseBoost )
    exec "normal %jA".l:date." - ".g:DoxygenToolkit_authorName
  endif
  exec "normal `d"
...
command! -nargs=0 DoxBoo :call <SID>DoxygenLicenseBooFunc()
